...
public class Questions
{
    ...
    public Question GenerateMCQ()
    {
        ...
        if (difficulty <= 0.3) // Ask easier questions
        {
            ...
            Question boxVolume = new Question($"A box has a height of {height}cm, a width of {width}cm, and a length of {width}cm. What is the volume of the box in metres?", easyWeightingTotal / 4, PhysicsEquations.CalcVolume(width / 100, height / 100, length / 100));
            double radius = (rnd.NextDouble() + 0.1) * rnd.Next(1, 75);
            Question cylinderVolume = new Question("A cylinder has a circular radius of {radius}m and a length of {length}m. What is the volume in metres?", easyWeightingTotal / 4, PhysicsEquations.CalcVolume(length, radius));
            ...
            Question molesCalc = new Question("Calculate the number of moles in a sample of {numParticles} particles.", easyWeightingTotal / 4, PhysicsEquations.NumberToMoles(numParticles));
            Question particlesCalc = new Question("Calculate the number of particles in {numMoles}mol of gas.", easyWeightingTotal / 4, PhysicsEquations.MolesToNumber(numMoles));
    ...
    public bool AnswerQuestion(double answer)
    {
        ...
        if (Math.Round(answer, 3) == Math.Round(CurrentAnswer.answer, 3) || Math.Round(answer, 3) + 0.001 == Math.Round(CurrentAnswer.answer, 3) || Math.Round(answer, 3) - 0.001 == Math.Round(CurrentAnswer.answer, 3))
        {
            correctQuestions++;
            difficulty += 0.1;
            return true;
